...
export class MicrosoftMailService {
  ...
  private formatMessage(msg: Record<string, unknown>, includeBody = false): OutlookMessage | OutlookMessageFull {
    const from = msg['from'] as Record<string, unknown> | undefined;
    const fromEmail = (from?.['emailAddress'] as Record<string, unknown>)?.['address'] as string ?? '';
  ...
  async sendMessage(to: string[], subject: string, body: string, cc?: string[]): Promise<{ status: string }> {
    ...
    if (cc?.length) {
      (message['message'] as Record<string, unknown>)['ccRecipients'] = cc.map(addr => ({ emailAddress: { address: addr } }));
    }
  ...
  async createDraft(to: string[], subject: string, body: string, cc?: string[]): Promise<{ id: string; status: string }> {
    const message: Record<string, unknown> = {
      subject,
      body: { contentType: 'Text', content: body },
      toRecipients: to.map(addr => ({ emailAddress: { address: addr } })),
  ...
  async moveMessage(messageId: string, destinationFolderId: string): Promise<{ id: string; status: string }> {
    const data = await this.post(`/me/messages/${messageId}/move`, { destinationId: destinationFolderId });
